...
const toPn = async (pn) => {
    ...
    try {
      noise.decodeFrame(data, frame => {
        try {
          ...
          logger.error({ err: frameErr }, "error processing decoded frame");
        }
      });
    } catch (err) {
      logger.error({ err }, "error decoding frame, skipping");
    }
  };
  const end = (error) => {
    if (closed) {
    ...
    try {
      ...
      logger.error({ err: e }, "error clearing timers");
    }
    try {
      ...
      logger.error({ err: e }, "error removing listeners");
    }
    ...
    try {
      ...
      logger.error({ err: e }, "error emitting close event");
    }
  };
  ...
  const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
    try {
      ...
      logger.error({ err }, "unexpected error in keep alive interval");
    }
  }, keepAliveIntervalMs));
