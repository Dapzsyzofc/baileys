...
const makeSocket = (config) => {
    ...
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
        ...
        /*
            ...
            increased tolerance to 25s to prevent false disconnects on slow networks
        */
        if (diff > keepAliveIntervalMs + 25000) {
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        // wait for WS to be open by polling, max 15 seconds
        for (let i = 0; i < 15 && !ws.isOpen; i++) {
        ...
        // retry sending pairing request up to 3 times
        let lastError;
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                ...
                lastError = err;
                logger.warn({ attempt: attempt + 1, trace: err.stack }, 'pairing code request failed, retrying...');
                if (attempt < 2) {
                    await (0, Utils_1.delay)(2000);
                    // wait for WS to reopen
                    for (let i = 0; i < 10 && !ws.isOpen; i++) {
                        await (0, Utils_1.delay)(1000);
                    }
                }
            }
    ...
    ws.on('CB:failure', (node) => {
        ...
        // only treat 401 (loggedOut) as true logout
        // other failures (503, 500, etc) should trigger reconnect, not permanent logout
        if (reason === 401) {
    ...
    ws.on('CB:ib,,offline_preview', (node) => {
        ...
        sendNode({
            tag: 'ib',
            attrs: {},
            content: [{ tag: 'offline_batch', attrs: { count: '1000' } }]
    ...
    process.nextTick(() => {
        // no buffering - emit events immediately for zero delay
    ...
    const unhandledRejectionHandler = (err) => {
        logger.error({ trace: err?.stack || err }, 'unhandled promise rejection caught — preventing crash');
    };
