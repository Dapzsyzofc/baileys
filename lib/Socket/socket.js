...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        ...
        // retry sending pairing request up to 5 times
        let lastError;
        for (let attempt = 0; attempt < 5; attempt++) {
            try {
                // ensure WS is open before sending
                if (!ws.isOpen) {
                    try { await waitForSocketOpen(); } catch (_e) { /* ignore, will retry */ }
                }
                if (!ws.isOpen) {
                ...
                lastError = err;
                logger.warn({ attempt: attempt + 1 }, 'pairing code request failed, retrying...');
                await (0, Utils_1.delay)(3000);
            }
        }
        throw lastError;
